...
def wave_mean_direction(a1: _T, b1: _T, unit: DirectionalUnit = 'degree', convention: DirectionalConvention = 'mathematical',name=None) -> _T:
...
def wave_directional_spread(a1: _T, b1: _T, unit: DirectionalUnit = 'degree', name=None) -> _T:
    spread = convert_unit(np.sqrt(2 - 2 * np.sqrt(a1 ** 2 + b1 ** 2)), to=unit, _from='rad')
    if name is not None:
...
def convert_unit(angle: _T, to: DirectionalUnit = 'degree', _from: DirectionalUnit = 'degree') -> _T:
    """
...
def convert_angle_convention(
    ...
    - nautical: 0 degree / rad is north, going to, measured positive clockwise.
...
def get_angle_convention_and_unit(dataarray, default_convention='mathematical', default_unit='degree'):
